...
L'obiettivo principale dello stage consiste nello sviluppo e nella realizzazione di una \emph{dashboard} per la gestione di dispositivi interconnessi (IoT).
Questo centro di controllo attraverso cui l'utente del sistema gestisce i dispositivi \emph{smart} presenti nella propria rete domestica dovrebbe permettere operazioni del tipo:
\begin{itemize}
  \item avvio/spegnimento di un dispositivo;
  \item monitoraggio dei dispositivi collegati;
  \item richiesta di dati per conoscere lo stato dei dispositivi (ad es. per una lampadina indicare se essa sia accesa o spenta, il suo assorbimento energetico, ecc.);
